package me.geoflow.core.jobs

import it.justwrote.kjob.Job
import it.justwrote.kjob.Prop

/**
 * Generic job for kjob to run a [me.geoflow.core.tasks.SystemTask]. Requires a [runId] and [pipelineRunTaskId] to find
 * the [me.geoflow.core.tasks.SystemTask] function and class then function, as well as a [runNext] flag to denote if
 * the user asked to keep running tasks after the first one is completed. Subsequent tasks are only run when next task
 * is a [me.geoflow.core.tasks.SystemTask] and the current task finished successfully
 *
